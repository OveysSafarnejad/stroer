...
class ModelAdminBase(admin.ModelAdmin):
    ...
    all the 'ModelAdmin' classes must be extended from this one instead of
    'admin.ModelAdmin'.
    """
    ...
    def __init__(self, model, admin_site):
        if len(self.list_display) == 0:
            self.list_display = [
                ...
                if type(
                    field) not in self._list_exclude and field.name not in self._list_display
            ]
    ...
    def get_detail_page(self, model_class, pk, items_title=None,
                        title='Details', button=False):
        """
    ...
    def get_detail_list_page(self, model_class, pk, title='Details',
                             button=False):
        """
        gets a link to detail page of a related entity.
